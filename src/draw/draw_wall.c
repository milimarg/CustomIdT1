...
int *delta_x, my_idt1 *world)
{
    *delta_x = (*delta_x == 0) ? 1 : *delta_x;
    ...
    wall->x1 = (wall->x1 > world->map.win_size.x - 1) ?
    world->map.win_size.x - 1 : wall->x1;
    wall->x2 = (wall->x2 > world->map.win_size.x - 1) ?
    world->map.win_size.x - 1 : wall->x2;
}
...
void draw_wall(coordinates_wall *wall, sfColor color, int s, my_idt1 *world)
{
    ...
    for (int x = wall->x1; x < wall->x2; x++) {
        ...
        y[0] = (y[0] < 1) ? 1 : y[0];
        y[1] = (y[1] < 1) ? 1 : y[1];
        y[0] = (y[0] > world->map.win_size.y - 1) ?
        world->map.win_size.y - 1 : y[0];
        y[1] = (y[1] > world->map.win_size.y - 1) ?
        world->map.win_size.y - 1 : y[1];
